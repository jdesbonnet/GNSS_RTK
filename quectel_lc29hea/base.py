...
if __name__ == '__main__':
    ...
    send_command(ser,"$PQTMCFGRCVRMODE,W,2") # set receiver to base mode
    send_command(ser,"$PQTMSAVEPAR") # save PQTM params to flash
# manually power cycle module
    send_command(ser,"$PAIR432,1") # output RTCM3 MSM7 messages
    send_command(ser,"$PAIR434,1") # output RTCM3 antenna position (1005)
    send_command(ser,"$PAIR062,0,01") # Enable NMEA GGA message
    x = 0
    y = 0
    z = 0

    send_command(ser,f"$PQTMCFGSVIN,W,2,0,0,{x},{y},{z}") # set base location in XYZ coords
    send_command(ser,"$PQTMSAVEPAR") # save PQTM params to flash
